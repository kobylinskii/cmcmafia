...
удаления в боте, в URL `/mafia/games/{id}`, номером `№14` в каждой карточке
...
Статистика победителя отдаётся **в зачёте номинации** (у MVP и мест — по всему
финальному столу): игры, победы, поражения, % побед, судейские, ЛХ, итог
таблицы. Показывается она не везде одинаково: MVP — главная номинация, идёт
первой и во всю ширину со всеми колонками; у ролевых карточек оставлены только
игры в зачёте и средний доп. балл, иначе четыре карточки в ряд читались как
таблица.

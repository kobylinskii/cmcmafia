...
Модерируются только пять полей текстового ввода: `full_name`, `nickname`,
`age`, `experience`, `bio` (`profile_change_service.MODERATED_FIELDS`).
...
админ разбирал бы стопку промежуточных вариантов. Значение хранится строкой
независимо от типа колонки и приводится к нему в момент применения; `NULL` —
осознанная очистка необязательного поля. Ник перепроверяется на занятость
**при применении**, а не только при подаче: очередь ничего не резервирует, и
за время ожидания ник мог занять другой человек.
...
GET  /api/bot/admin/sessions/day-cards              ?game_type= — без турнирных и без оценённых
GET  /api/bot/admin/sessions/by-day                 ?day=       — тот же набор игр
GET  /api/bot/admin/sessions/pending-review
GET  /api/bot/admin/sessions/awaiting-confirmation  — прошедшие игры без ответа «состоялась?»
POST /api/bot/admin/sessions/{id}/played            — «Игра проведена»: scheduled -> played
GET  /api/bot/admin/sessions/locations              — места прошлых игр для выбора кнопкой
...
- **played** — бот-админ нажал «✅ Игра проведена» в карточке игры
  (`POST /api/bot/admin/sessions/{id}/played` → `game_service.mark_session_played`).
  ...
  Прошедшие неподтверждённые игры видны в боте отдельным разделом
  («⏳ Подтвердить проведение», `sessions/awaiting-confirmation`); ответ «не
  состоялась» — это `DELETE` игры вместе с записями. Турнирные слоты в этой
  очереди не участвуют — они оцениваются внутри своего этапа.
...
1. **Бот-игра**: `POST /api/bot/admin/sessions` (funky/training) → игроки
   записываются через `registrations`/`reserves` → бот-админ подтверждает
   проведение и игра становится `played` → сайт-админ открывает
...
backend/app/
  ...
  services/            — бизнес-логика без привязки к HTTP
    ...
    schedule_admin_service.py — bulk-создание слотов дня, конфликты, обзор по дням,
                                 места прошлых игр (бот)
...
профиля, новое место проведения и поиск человека при выдаче прав. Дата
игрового дня выбирается в календаре (`inline.calendar_keyboard`, прошедшие дни
некликабельны), часы — сеткой целых часов, место — из мест прошлых игр
(`sessions/locations`). Всё, что человек всё-таки набрал, бот удаляет из чата
сразу после разбора (`ui.consume_input`): значение он и так показывает в
экране раздела, а второй его копией история только засоряется.
...
bot/
  ...
  app/notifier.py                  — опрос двух очередей решений (заявки 3.7, правки профиля 3.8)
  ...
  app/handlers/
    ...
    profile.py                         — карточка профиля, правка всех полей кроме фото
                                          (текстовые уходят на проверку, раздел 3.8),
                                          повторная заявка после отказа, краткая статистика
    schedule.py                          — запись на игру (с авто-резервом), свои регистрации, состав
    admin.py                              — игровые дни, подтверждение проведения, анонс, бот-админы
```

`bot/tests/` — два уровня, оба без Telegram и без сети: `test_bot_logic.py` —
чистая логика (нарезка дня по часам, календарь без прошедших дней, граница
«прошедшая игра» в клубной зоне, развилка «пора подтверждать», семантика `ack`),
`test_bot_flow.py` — прогон диалогов через настоящий `Dispatcher` с заглушками
`Bot` и `ApiClient`: регистрация, запись с попаданием в резерв, создание
игрового дня кнопками, подтверждение проведения и рассылка анонса.
...
Бот **не создаёт и не показывает турнирные игры** — ни в меню записи, ни в
админской пачке слотов дня: вся сетка турниров ведётся на сайте (раздел 7).

**Оценённых игр в расписании бота тоже нет** (`_MANAGED_GAMES` в
`schedule_admin_service`): после `rated` состав, баллы и исход правятся только
на сайте, а «Игровые дни» иначе прирастали бы каждым отыгранным днём и не
сокращались никогда. День, все игры которого оценены, исчезает из списка
целиком.
...
человек кнопкой в боте (раздел 5).

**В процессе бота**: `app/notifier.py:notifier_loop()` раз в
`CONFIRMATION_POLL_SECONDS` (по умолчанию 60) забирает у API две независимые
очереди решений админа — по заявкам на вступление (раздел 3.7) и по правкам
профиля (раздел 3.8) — и рассылает их в Telegram. Очереди разделены, чтобы
недоставленное решение одного вида не задерживало другое. Здесь
разделение зеркальное: сообщение в Telegram — это уже не состояние данных, и
слать его обязан тот, у кого есть токен бота.
...
- `test_e2e_flow.py` — сквозной прогон: админка заводит игроков и турнирную
  ...
  авто-промоушеном.
- `test_review_flow.py` — цикл «сессия из бота → подтверждение проведения →
  форма оценки на сайте → rated», включая отказ подтвердить ещё не начавшуюся
  игру и удаление несостоявшейся.
- `test_bot_reserve_and_broadcast.py` — авто-резерв (первые `max_players` за
  столом, остальные в очередь по порядку, у штаба очереди нет) и подбор
  анонса: окно недели, вычет уже записанных и отклонённых.
...
чистая логика: нарезка игрового дня по часам, календарь без прошедших дней,
граница «прошедшая игра» в клубной зоне, развилка «игру пора подтвердить»,
...
текста из чата, создание игрового дня кнопками, подтверждение проведения и
рассылка анонса.

...
GET  /api/admin/schedule/days                          ?game_type= — игровые дни со счётчиками
                                                        (игры, ждут подтверждения, места за столами,
                                                         штаб, резерв, сколько людей придёт)
...
### 5.1.1 Насколько собрался игровой день

Карточка дня (`GET /api/admin/schedule/days`) отвечает на единственный вопрос,
ради которого в расписание и заходят: собираются ли столы. Кроме числа игр она
несёт набор дня — `players`/`seats` (занятые места за столами и сколько их
всего), `staff`, `reserves` и `people`.

`players` и `people` считаются по-разному намеренно: записанный на три игры
подряд занимает **три места** за столами, но придёт **один раз**. Первое
показывает, добираются ли столы, второе — сколько народу ждать в аудитории.
Поэтому `people` считается отдельным запросом по `DISTINCT player_id` (со
штабом и резервом), а не тем же `GROUP BY`, которым считаются места.
...
всегда стоит день недели: игровые дни клуба повторяются именно по дням недели,
и без подписи админ сверяется с календарём на каждой строке.

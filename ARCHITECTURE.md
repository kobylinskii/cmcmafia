...
GET  /api/bot/admin/broadcast/day                   ?day=&audience=absent|registered — игры дня +
                                                     кого звать (absent) или кому напомнить (registered)
...
состав ровно затем, чтобы решить, идти ли вообще.
...
**Напоминание о сегодняшних играх** — кнопка «⏰ Напомнить про сегодня»,
та же ручка с `audience=registered`: получатели — только записавшиеся на
сегодня, включая резерв, одно сообщение на человека со всеми играми дня и их
местами. Раньше это уходило само за три часа до первой игры дня; автоматика
...
этому. В `callback_data` и в API день едет по-прежнему голым токеном
`ДДММГГГГ` (`keyboards/inline.day_token` и обратная ему `day_from_token`).

...
О самой новой правке админам сайта пишет бот — той же очередью
...
GET  /api/bot/admin-notifications                     — что появилось на проверку + telegram_id админов сайта
...
bot/
  ...
  app/notifier.py                  — опрос трёх очередей: решения игрокам (заявки 3.7, правки 3.8)
                                     и новые заявки/правки админам сайта (3.7)
...
Причина, по которой всё это делает бот, а не бэкенд, во всех трёх случаях одна:
...
отсутствовать (ни у одного `is_site_admin` нет Telegram) — тогда очередь просто
ждёт, а заявку всё равно видно на вкладке «Обзор».

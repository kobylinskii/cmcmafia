...
**Оповещение админа.** Симметрично: о новой заявке админам сайта тоже пишет
бот. Свежая `pending`-строка с пустым `confirmation_admin_notified_at` —
очередь `GET /api/bot/admin-notifications` (там же `telegram_id` админов сайта,
которым писать), бот рассылает и ставит метку `ack`'ом. `resubmit` метку
снимает — повторная подача для админа новое событие. Получателей нет (ни у
одного `is_site_admin` не привязан Telegram) — очередь не трогается: адресат
может появиться позже. Разбирается заявка всё равно на вкладке «Обзор», так что
пропущенное сообщение не теряет её — только задерживает реакцию.
...
игрок, и без неё отказ выглядит поломкой. Те же операции остались и на сайте,
во вкладке «Обзор»: это одни и те же сервисы, вызванные из двух мест.
Уведомление уходит **каждому** админу с привязанным Telegram (и сайта, и
бота — `admin_notification_service.admin_recipients`) своей копией; решает
кто-то один, у остальных нажатие получает 409 «уже рассмотрено» и их копия
...
удаления в боте, в URL `/mafia/games/{id}`). Поэтому нумерация ведётся не
...
сохранения уходят на список — но внешняя ссылка на `/mafia/games/{id}`,
...
игравшие, у остальных рейтинга просто нет; в таблице им и рисуется прочерк
вместо места и рейтинга. Модерация регистраций сильнее поиска: неподтверждённый
...
bot/
  ...
  app/handlers/
    ...
    moderation.py                      — решение админа кнопкой под уведомлением (3.8):
                                          подтвердить/отклонить с причиной, не выходя в браузер
...
**Своя запись остаётся в списке с галочкой.** `GET /api/bot/sessions/open`
больше не вычитает игры, на которые человек записан, — вместо этого отдаёт
`my_role`, и строка показывается как «✅ 18:00 · Фанки · вы записаны (игрок)».
Раньше игра из списка исчезала, и нажатие читалось как «слот куда-то делся», а
не «место занято мной». Повторное нажатие на такую строку отвечает подсказкой,
что отменяют запись в «📋 Мои регистрации».
...
«Игры → Расписание» (раздел 5). В боте осталось только то, чего без Telegram
не сделать: назначить админа человеку, которого знают по @username (его
`telegram_id` появится лишь при первом `/start`), и две рассылки — их шлёт
тот, у кого есть токен бота. Нажатие на кнопку планировщика в экране,
...
отсутствовать (ни у одного админа нет Telegram) — тогда очередь просто ждёт, а
заявку всё равно видно на вкладке «Обзор».

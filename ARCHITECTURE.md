...
games (
    ...
    day_reminder_sent_at   TIMESTAMPTZ,           -- наследие автоматического «сегодня игры»;
                                                  -- не пишется и не читается (раздел 12)
...
Длину «опыта» бот держит короче схемы: `EXPERIENCE_MAX_LENGTH` = 50 символов
(`bot/app/handlers/profile.py`) против 2000 в API. Это строка в карточке
игрока — и в боте, и на сайте, — а не рассказ; API оставлен свободнее,
чтобы уже сохранённые длинные значения не стали невалидными.
...
**Первое решение — окончательное**, и у заявок тоже
(`player_confirmation_service._ensure_pending`). Раньше эта проверка была
только у правок профиля, а `confirm`/`reject` игрока меняли статус из любого
состояния: если один админ подтверждал, а второй в ту же минуту отклонял,
побеждало последнее нажатие, и игрок получал два взаимоисключающих
сообщения подряд. Теперь второй админ видит «Заявка уже рассмотрена», а
вернуть отклонённую заявку на проверку может сам игрок — кнопкой «Отправить
на повторную проверку» (`resubmit`).
...
GET  /api/admin/schedule/days                          ?game_type= — игровые дни со счётчиками
                                                        (игры, ждут подтверждения, сколько людей
                                                         записано на день)
GET  /api/admin/schedule/sessions                      ?day=ДД.ММ.ГГГГ — игры одного дня
POST /api/admin/schedule/cleanup                       — убрать прошедшие слоты без оценки (вход в админку)
POST /api/admin/schedule/plan/preview                  — во что развернётся план и что уже занято
                                                        (занято = планировщик там что-то показывает)
...
GET  /api/bot/admin/broadcast/day                   ?day=&audience=absent|registered[&game_ids=1,2] —
                                                     игры дня + кого звать (absent) или кому напомнить
                                                     (registered; game_ids сужает до отмеченных игр)
...
- `needs_rating = false` — слот существует только ради записи в боте
  ...
  игры прошло, слот уходит из расписания
  (`schedule_admin_service._active_schedule_filters`), потому что нажимать по
  нему больше нечего, — **и удаляется из базы вместе со своими записями**
  (`purge_finished_unrated`, ниже). Пока его время не наступило, он живёт
  обычной жизнью: запись в боте, состав, список на пропуск.
...
**Прошедший слот без оценки удаляется.** Раньше такая строка оставалась в
базе навсегда: невидимая нигде — ни в расписании, ни на сайте (там только
`rated`), ни в боте (там только будущие игры), — но занимающая своё время в
планировщике и своё место в нумерации. Теперь её убирает
`schedule_admin_service.purge_finished_unrated()` — обратная сторона
`_active_schedule_filters`: за игрой каскадом уходят её `registrations` и
`reserves`, а нумерация пересобирается.

Запускает уборку **вход в админку**: `POST /api/admin/schedule/cleanup`,
который дёргает `components/admin/admin-shell.tsx` сразу после проверки прав.
Фоновой задачи под это нет намеренно — их у API нет вовсе (см. `app/main.py`),
а GET-ручки расписания ничего не удаляют: чтение не должно менять данные.
Если что-то удалилось, панель делает `router.refresh()`: номера игр после
уборки сжались, и открытая страница обязана перечитать данные.

Следствие для списка на пропуск: он строится по текущей пропускной неделе, и
уже сыгранные дни этой недели из него уходят вместе с играми. Список заказывают
до начала недели, так что теряется только история.

**Ушедшая из расписания игра не занимает своё время.** Проверка пересечений
(`check_conflicts`) смотрит тем же фильтром, что и сам планировщик: занято
только то время, на котором он что-то **показывает**. Без этого время
навсегда занимала игра, которой в расписании уже нет, — оценённая, турнирная
или прошедшая неоцениваемая: форма планирования отвечала «на это время игры
уже созданы», а в этом дне не показывала ни одной, и снять такую занятость
было нечем.

### 5.1.1 Наберётся ли игровой день

Карточка дня (`GET /api/admin/schedule/days`) отвечает на единственный вопрос,
ради которого в расписание и заходят: **наберутся ли одиннадцать человек** —
десять за стол и ведущий. Поэтому счётчик набора там ровно один: `people`,
число РАЗНЫХ людей, записанных на этот день в любой роли, включая резерв.

Считается он отдельным запросом по `DISTINCT player_id`, а не `GROUP BY` по
играм: записанный на три игры подряд — это три строки записи, но один человек,
и складывать их нельзя.

Мест по столам и размера штаба в карточке нет намеренно: «10/50 за столами» и
«штаб 0/15» не отвечают ни на один вопрос, который задают этому экрану, — кто
на каком столе и хватает ли судей, видно внутри дня.

Показывает это `components/admin/schedule/day-card.tsx` — одна строка на два
места: список «Игровые дни» в расписании и блок «Ближайшие игры» на «Обзоре»
(`components/admin/upcoming-days.tsx`, ближайшие шесть дней). Рядом с датой
всегда стоит день недели, и целиком («понедельник», не «пн»): игровые дни клуба
повторяются именно по дням недели, и без подписи админ сверяется с календарём
на каждой строке.
...
backend/app/
  ...
  services/            — бизнес-логика без привязки к HTTP
    ...
    schedule_admin_service.py — планирование слотов дня (шаг и количество), конфликты
                                 по времени (только с тем, что планировщик на этом
                                 времени показывает), обзор по дням, места прошлых игр
    broadcast_service.py      — игры и получатели трёх рассылок бота: неделя, сбор на
                                 день, напоминание записавшимся (раздел 11.4)
...
frontend/src/
  ...
  components/
    ...
    admin/schedule/                  — планировщик: schedule-panel (дни, день, очередь
                                        подтверждения), day-card (строка игрового дня: день
                                        недели, дата, полоса набора -- общая у расписания и
                                        дашборда), plan-form (пачка слотов с предпросмотром
                                        времён), session-card (правка слота, «проведена /
                                        не состоялась», состав и резерв)
    admin/upcoming-days.tsx          — дашборд ближайших игровых дней на «Обзоре»
...
bot/
  ...
  app/handlers/
    ...
    schedule.py                          — запись на игру (с авто-резервом), свои регистрации,
                                          состав дня, вход по ссылке из рассылки
    admin.py                              — назначение админов и четыре рассылки (11.4):
                                             анонс недели, сбор на день, напоминание на
                                             сегодня и произвольное сообщение; расписание
                                             уехало на сайт (раздел 5)
```
...
запись с попаданием в резерв, состав дня, приход по ссылке-deep-link из
рассылки, все рассылки и подсказка на кнопках уехавшего планировщика.
...
Сбрасывать диалог поэтому нужно через `ui.clear_state()`, а не голым
`state.clear()`: тот уносит **все** данные FSM, включая `screen_message_id`,
и следующий `open_screen` уже не знает, что удалять, — в чате остаётся второй
экран с рабочими кнопками. Виднее всего это было на `/start` по
ссылке-deep-link из рассылки: старое меню не исчезало, а новый экран
приходил под ним.
...
**«📋 Мои регистрации» устроены так же, как запись: день → игры этого дня.**
Вкладок «Предстоящие»/«Прошедшие» больше нет. Прошедшие записи в раздел не
попадают вовсе: отменять в них нечего, а сыгранное видно в статистике профиля
и на сайте. Экран дня открывает карточку игры с составом и отменой, и «Назад»
с неё ведёт в тот же день (день берётся из самой игры, в `callback_data` его
нет). Кнопки прошлой версии (`mr:list:active|completed`) продолжают работать
— они ведут в новый первый экран, как `sg:types` в записи.

**Состав показывается за весь день.** На экране игр дня есть кнопка
«👥 Кто записан»: она показывает **один общий список** на весь день —
«ведущие/судьи», игроки и резерв, — а не столбик составов по играм. Штаб на
разделы не делится нигде: это три места, которые набирают вместе, и так же
их считает строка игры в списке (`hosts + judges` из трёх). Те же разделы у
состава одной игры в «📋 Мои регистрации». Клуб
играет вечер целиком, человек смотрит, кто сегодня придёт, а не кто сядет за
какой из трёх столов; записанный на две игры подряд считается один раз.
Раньше состав был виден только у своей игры, из «📋 Мои регистрации», — то
есть узнать, кто сегодня придёт, мог лишь тот, кто уже записался, а смотрят
состав ровно затем, чтобы решить, идти ли вообще. Формат и роль едут в
`callback_data` этой кнопки (`sg:who:<день>:<формат>:<роль>`) не ради отбора
игр — состав дня всегда полный, — а ради «Назад»: вернуть надо на тот же
экран игр, а не в начало цепочки, где заново спросят формат и роль.
...
сколько человек заблокировали бота.

В самом анонсе — только дни и сколько в каждом игр («📅 пт 19.09.2026 —
3 игры»), а под ним по кнопке-ссылке на каждый день. Строка на каждую игру с
местом и свободными местами уезжала за экран уже на трёх игровых днях, а
решение всё равно принимается на уровне «в какой день я иду»; подробности
человек видит на экране записи, куда его и приводит кнопка. Кнопки —
**ссылки** `https://t.me/<бот>?start=day<ДДММГГГГ>`, а не `callback_data`:
сообщение рассылки живёт в чате неделю и его пересылают друзьям, а у
пересланного сообщения callback не работает вовсе. `/start` с такой
полезной нагрузкой разбирает `handlers/schedule.start_on_day` и открывает
сразу список игр этого дня.

**Сбор на конкретный игровой день** — кнопка «📣 Собрать на игровой день».
Админ выбирает день, бот показывает предпросмотр (время, место и свободные
места по каждой игре — зовут сюда тех, кто ещё не записан, и им это как раз
нужно) и число получателей. Аудитория — `GET /api/bot/admin/broadcast/day`
с `audience=absent`: весь клуб, кроме тех, у кого на этот день уже есть
запись или резерв. Под сообщением — та же кнопка-ссылка, но одна, на этот
день.

**Напоминание о сегодняшних играх** — кнопка «⏰ Напомнить про сегодня», та
же ручка с `audience=registered`: одно сообщение на человека со списком игр и
их мест. Игры дня показываются галочками, и админ снимает те, что сегодня не
состоятся; отмеченные едут в ручку как `game_ids`, и получатели считаются по
ним же — писать «сегодня играем» тому, чья игра отменена, было бы враньём.
По умолчанию отмечены все: расписание — это уже решение клуба, а отменяют
игру реже, чем проводят. Раньше это уходило само за три часа до первой игры дня; автоматика
не знала ни про отмену в последний момент, ни про то, что клуб уже всё
обсудил в чате, — теперь момент выбирает админ. Кнопки под напоминанием нет
намеренно: получатель уже записан, состав и отмена живут в «📋 Мои
регистрации».

**Даты человеку показываются с днём недели** («пт 19.09.2026»): дата сама по
себе не говорит, пятница это или суббота, а день игры выбирают именно по
этому. На кнопках год опускается (`api_client.day_button_label` — «пт 19.09»):
кнопки дней стоят по две в ряд, и полная дата в них не помещалась — Telegram
обрезал её многоточием. В `callback_data` и в API день едет по-прежнему голым
токеном `ДДММГГГГ` (`keyboards/inline.day_token` и обратная ему
`day_from_token`).
...
`telegram_id` появится лишь при первом `/start`), рассылки — их шлёт тот,
...
`CONFIRMATION_POLL_SECONDS` (по умолчанию 60) забирает у API три независимые
...
**Четвёртой очередью было автоматическое «сегодня игры»** за три часа до
первой игры клубного дня. Её больше нет: напоминание рассылает админ кнопкой
«⏰ Напомнить про сегодня» (раздел 11.4), потому что решение «напоминать или
не надо» зависит от того, что происходит в клубе, а не от часов на сервере.
Вместе с очередью ушли `services/day_reminder_service.py` и
`GET /api/bot/day-reminders`; колонка `games.day_reminder_sent_at` осталась в
схеме — её никто не пишет и не читает, но и ронять её миграцией ради этого
незачем.
...
- `test_review_flow.py` — цикл «слот в расписании → подтверждение проведения →
  ...
  удаление несостоявшейся, обе стороны флага `needs_rating` (раздел 5.1) и то,
  что время, освобождённое ушедшей из расписания игрой, снова можно занять.
...
- `test_day_broadcast.py` — рассылки по одному игровому дню (раздел 11.4):
  разделение аудиторий (`absent` / `registered`), игры с закрытой записью в
  напоминании и их отсутствие в сборе, изоляция дней друг от друга.

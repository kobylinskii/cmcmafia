...
STEP_ROLES = (
    ...
    "Это влияет на то, на какие места пускает запись."
)
STEP_NICKNAME = (
...
async def nickname_received(message: Message, state: FSMContext, api: ApiClient, bot: Bot) -> None:
    ...
    await open_screen(
        ...
        state,
        f"Готово, {nickname}! 🎉\n\n"
        "⏳ Заявка ушла на проверку администратору. Записываться на игры можно уже сейчас, "
        f"в рейтинге на сайте появитесь после подтверждения.{admin_note}",

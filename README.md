...
Сайт клуба спортивной мафии и Telegram-бот для записи на игры —
...
Скрипт печатает логин и временный пароль — входить на `/admin/login`.
...
игрок меняет **через проверку админа**: правка ждёт решения в `/admin`
...
**Расписание игр ведётся на сайте**, в `/admin` → «Игры» → вкладка

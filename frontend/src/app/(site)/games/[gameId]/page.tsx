...
async function getGame(gameId: string): Promise<GameOut | null> {
  try {
    ...
    // 404, и /games/abc (422 от бэкенда) вместе с номером, не влезающим
...
export default async function GameDetailPage({ params }: PageProps<"/games/[gameId]">) {
  ...
  return (
    <Container className="py-14">
      <Link href="/games" className="inline-flex items-center gap-1.5 text-sm text-ink-400 hover:text-ink-100">
        <ArrowLeft size={16} />
        Все игры
      </Link>

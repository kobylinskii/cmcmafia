...
/** «среда» -- день недели целиком, по клубному времени.
 *
 ...
 * календарём на каждой строке расписания. Целиком, а не «ср»: в админке есть
 * место, а сокращение читается медленнее. */
export function formatWeekday(iso: string): string {
  return new Intl.DateTimeFormat("ru-RU", { weekday: "long", timeZone: CLUB_TZ }).format(
    new Date(iso)
  );
}

/** Первая буква заглавной -- для начала строки. Intl отдаёт день недели строчным
 * («понедельник»), и это правильно в середине фразы («Игры в понедельник»), но
 * не там, где с него строка начинается. */
export function capitalize(value: string): string {
  return value.charAt(0).toUpperCase() + value.slice(1);
}

/** «среда, 07.10.2026» из клубной даты «07.10.2026» -- в этом виде день приходит

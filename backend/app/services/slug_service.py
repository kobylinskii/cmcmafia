...
RESERVED_SLUGS = {
    ...
    "_next",
    "mafia",
    # Страницы игроков живут прямо в корне (/<slug>), поэтому заняты и
    # служебные маршруты фронтенда: прокси картинок и сброс кеша. Next
    # разрешает статический сегмент раньше динамического, так что игрок с
...
def suggest_slug(nickname: str, db: Session, *, scope: str = "player") -> str:
    ...
    и они независимы: игроки живут в /[slug], турниры -- в
    /tournaments/[slug]. Раньше занятость проверялась только по игрокам,

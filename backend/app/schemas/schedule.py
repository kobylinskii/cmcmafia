...
class ScheduleDayOut(BaseModel):
    ...
    # Сколько РАЗНЫХ людей записано на этот день -- в любой роли и включая
    # резерв. Единственный счётчик набора, который что-то решает: игра идёт,
    # когда пришло хотя бы десять игроков и ведущий. Места по столам и размер
    # штаба тут не нужны -- записанный на три игры подряд человек всё равно
    # один, а какой именно стол он займёт, видно внутри дня.
    people: int = 0

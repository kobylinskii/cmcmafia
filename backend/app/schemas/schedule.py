...
class ScheduleDayOut(BaseModel):
    ...
    # Набор дня: сколько мест за столами занято и сколько их всего. Считается
    # по записям всех игр дня, поэтому записанный на три игры подряд даёт
    # три занятых места -- вопрос у админа ровно такой: собираются ли столы.
    players: int = 0
    seats: int = 0
    # Штаб (ведущий + двое судей на игру) и очередь -- отдельно: пустой штаб
    # это «игру некому вести», а не «мало народу».
    staff: int = 0
    reserves: int = 0
    # Сколько РАЗНЫХ людей придёт в этот день, включая штаб и резерв.
    people: int = 0
